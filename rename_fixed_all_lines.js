...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    // 保留所有专线类节点：匹配地名 + IEPL/IPLC/专线 + 倍率后缀
    if (/(IEPL|IPLC|专线)/i.test(e.name)) {
      const locationMatch = e.name.match(/(广港|深港|沪港|京港|沪日|京新|杭新|中日|广新|港新|深新|深美|深日|广日|深韩|沪韩|.*?)/);
      const line = /IPLC/i.test(e.name) ? "IPLC" : "IEPL";
      const suffixMatch = e.name.match(/x\d+|\d+x|\d+倍|\d+×|\d{1,3}(\.\d+)?x?/i);
      const cleanSuffix = suffixMatch ? " " + suffixMatch[0].replace(/[^\dx×倍.]/gi, "") : "";
      const location = locationMatch ? locationMatch[0] : "专线";
      e.name = location + " " + line + cleanSuffix;
      return;
    }
