...
const nameclear =
  /(套餐|到期|有效|剩余|版本|已用|过期|失联|测试|官方|网址|备用|群|TEST|客服|网站|获取|订阅|流量|机场|下次|官址|联系|邮箱|工单|学术|USE|USED|TOTAL|EXPIRE|EMAIL)/i; // Removed 专线 to prevent filtering
// prettier-ignore
...
const rurekey = {
  ...
  G: /\d\s?GB/gi,
  Esnc: /esnc/gi,
  爱尔兰: /(?<!IEPL|IPLC)\bIE\b/i, // Prevent matching "IE" in "IEPL" or "IPLC"
};
...
function operator(pro) {
  ...
  if (inname !== "") {
    ...
    // Prioritize ZH for Chinese characters to avoid EN list misidentification
    inputList = [ZH, FG, QC, EN];
    if (/[\u4E00-\u9FFF]/.test(pro[0]?.name)) {
      inputList = [ZH]; // Force ZH if Chinese characters are detected
    }
  }
  ...
  if (clear || nx || blnx || key) {
    pro = pro.filter((res) => {
      ...
      const shouldKeep =
        !(clear && nameclear.test(resname) && !BLKEYS.some((k) => resname.includes(k))) && // Retain if keyword is in BLKEY
        !(nx && namenx.test(resname) && !BLKEYS.some((k) => resname.includes(k))) && // Retain if keyword is in BLKEY
