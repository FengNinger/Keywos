...
const rurekey = {
  ...
  "Hong Kong": /Hongkong|HONG KONG|(深|沪|呼|京|广|杭)港(?!.*(I|线))/gi,
...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    // 预处理：保护 IEPL 等完整术语
    if (/IEPL/i.test(e.name)) {
      e.name = e.name.replace(/IEPL/i, "PROTECTED_IEPL");
    }
    ...
    // 恢复 IEPL
    if (/PROTECTED_IEPL/.test(e.name)) {
      e.name = e.name.replace(/PROTECTED_IEPL/, "IEPL");
    }
