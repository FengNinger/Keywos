...
const nx = inArg.nx || false,
  ...
  nm = inArg.nm || true; // Default to true to retain unmatched nodes
...
function operator(pro) {
  ...
  if (clear || nx || blnx || key) {
    pro = pro.filter((res) => {
      ...
      const shouldKeep =
        ...
        !(key && !keya.test(resname)); // Relaxed key filter to only require region match
      if (debug && !shouldKeep) {
        ...
        console.log(`Key: ${key && !keya.test(resname)}`);
      }
      return shouldKeep;
    });
  }
  ...
  pro.forEach((e) => {
    ...
    if (findKey?.[1]) {
      ...
      e.name = FNAME + FGF + e.name; // Retain original name if no region matched
      if (debug) {
        console.log(`No region matched for ${e.name}, retaining with prefix: ${FNAME}`);
      }
    }
    if (debug) {
